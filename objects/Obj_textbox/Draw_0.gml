...
//textbox_y = camera_get_view_y(view_camera[0]) + 5;
//}else{
textbox_x = camera_get_view_x(view_camera[0]);
textbox_y = camera_get_view_y(view_camera[0]) + 147;
//}
...
{
	//how many characters are on each page
	text_length[p] = string_length(text[p]);
	
	//get the x position for textbox
		//character on left
		text_x_offset[p] = 93;
		portait_x_offset[p] = 8;
		
		//character on right
		if speaker_side[p] == -1
		{
			text_x_offset[p] = 6
		portait_x_offset[p] = 196
			
		}
		
		//no character talking (center screen textbox)
	...
	//setting induvidual chatracters and finding where lines of text should break
	for (var c = 0; c < text_length[p]; c++)
	{
		
		var _char_pos = c+1;
		
		//store induvidual characters in the char array
	...
	//add to the x y coords based on new info
...
if option_pos == op
{
draw_sprite(spr_textbox_sword, 0, _txtb_x, _txtb_y - _op_space*option_number + _op_space*op);
}
...
for(var c = 0; c < draw_char; c++)
{
	...
	//the text
	draw_text_color(char_x[c,page] + _shake_x, char_y[c,page] + _float_y + _shake_y, char[c, page], col_1[c, page],col_1[c, page],col_1[c, page],col_1[c, page],1);
