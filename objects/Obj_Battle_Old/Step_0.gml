...
if (cursor.active)
{
	with (cursor)
	{
		...
		if (_moveH == -1) targetSide = Obj_Battle_Old.partyUnits;
		if (_moveH == 1) targetSide = Obj_Battle_Old.enemyUnits;
		
		//verify target list
		if (targetSide == Obj_Battle_Old.enemyUnits)
		{
		...
		if (_keyConfirm)
		{
			with (Obj_Battle_Old) BeginAction(cursor.activeUser, cursor.activeAction, cursor.activeTarget);
